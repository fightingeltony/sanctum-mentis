...
**Bestehende Lectios (14 registriert in `data.ts`):** `hard-problem` (Geist) · `findest-du-oder-machst-du` (Selbst) · `wenn-die-welt-wackelt` (Realismus) · `warum-sollst-du` (Ethik) · `wenn-nichts-vorgegeben` (Existenzialismus) · `warum-gehorchst-du` (Politische Philosophie) · `ruhe-oder-rausch` (Lebenskunst) · `der-weg-des-menschen` (Begegnung — Ein-Werk-Lectio, kontemplativ-vertiefend) · `ist-der-andere-hoelle-oder-heimat` (Begegnung — konkurrierend-konfrontativ) · `verstehen-oder-weitergehen` (Wandlung — narrativ-historisch) · `stell-die-frage-anders` (Gut und Böse — destruktiv-aufbauend) · `wer-bist-du-wenn-du-alles-weglaesst` (Selbst — konkurrierend-konfrontativ, **ton: erzählend-erfahrend** — erster Test des erzählenden Tons, Vergleichsfall zur archivierten expositorischen Fassung `wer-beobachtet`, s.u.) · `vom-wissen-zum-glauben` (Verwandlung — erzählend-erfahrend, L3, 6 Stationen: Augustinus→Stoa→Rilke→Eckhart→James→Jung) · `annehmen-oder-ueberwinden` (Selbstverhältnis — konkurrierend-konfrontativ, **ton: erzählend-erfahrend**, L2, 4 Stationen: Rogers→Nietzsche→Marc Aurel→Buddhismus).

**Archivierte Lectio-Dateien:** 7 expositorische Vorläufer archiviert nach `archiv/lectio-expositorisch/` (Repo-Ebene, außerhalb von `data/lectio/` — nicht registriert, keine Route, Inhalt unverändert): `warum-gehorchst-du-expositorisch`, `der-weg-des-menschen-expositorisch`, `ist-der-andere-hoelle-oder-heimat-expositorisch`, `ruhe-oder-rausch-expositorisch`, `verstehen-oder-weitergehen-expositorisch`, `wenn-nichts-vorgegeben-expositorisch`, `wer-beobachtet-expositorisch` (abgelöst durch `wer-bist-du-wenn-du-alles-weglaesst`, archiviert 28.6.).
